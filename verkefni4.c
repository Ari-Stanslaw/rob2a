...
task main()
{
	...
	while(SensorValue(lightSensor) > 200  && vexRT[Btn8D] != 1)
	{
		...
		if(SensorValue(sonarSensor) < 50  || SensorValue(sonarSensor) == -1 && vexRT[Btn8D] != 1)		// Loop while robot's Ultrasonic sensor is further than 20 inches away from an object
		{                                                                         // || (or) it is '-1'.  (-1 is the value returned when nothing is in it's visable range)
